...
def search_industrial_installations(
    ...
    """Searches DEFRA environment API industrial installations registry"""
    try:
        url = f"{__BASE_URL}/industrial-installations/registration.json"
        params = {}
        if number_search:
        ...
        for item in result.json()["items"]:
            item_info = {
                ...
                "documentURL": item.get("documentURL", None),
            ...
            # Handle activity (can be single object or array)
                ...
                if isinstance(item.get("activity"), list):
                    ...
                    for activity in item.get("activity"):
                        activity_info = {
                            ...
                            "comment": activity.get("comment", None),
                        }
                        item_info["activity"].append(activity_info)
...
def search_for_industrial_installation(
    id: str,
) -> str:
    """Searches DEFRA environment API industrial installations registry for a particular registration"""
    try:
        url = f"{__BASE_URL}/industrial-installations/registration/{id}.json"
